...
# Adapted from the static InterpIoU implementation by Haoyuan Liu and Hiroshi Watanabe:
...
def bbox_interpiou(
    ...
    xywh: bool = True,
    interpolation_alpha: float = 0.98,
    eps: float = 1e-7,
) -> torch.Tensor:
    """Calculate static InterpIoU scores between predicted and target bounding boxes."""
    if xywh:
...
class InterpIoUBboxLoss(BboxLoss):
    """Bounding-box criterion using static InterpIoU while retaining the baseline DFL path."""

    def _bbox_regression_loss(self, pred_bboxes: torch.Tensor, target_bboxes: torch.Tensor) -> torch.Tensor:
        """Return the per-box static InterpIoU regression loss with the paper's fixed coefficient."""
        return 1.0 - bbox_interpiou(
            pred_bboxes,
            target_bboxes,
            xywh=False,
...
class v8DetectionInterpIoULoss(v8DetectionLoss):
    """Detection criterion replacing only positive-box CIoU regression with static InterpIoU."""

    def __init__(self, model, tal_topk: int = 10):
        """Initialize detection loss with unchanged TAL, DFL, weighting, and gains."""
        super().__init__(model, tal_topk)
        self.bbox_loss = InterpIoUBboxLoss(self.reg_max).to(self.device)
