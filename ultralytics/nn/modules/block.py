...
class C3k2CrossPSA(C3k2):
    """C3k2 extension that refines only its final processed branch with one CrossPSA block."""
    ...
    def __init__(
        ...
        """Initialize the key-preserving C3k2 path and its new CrossPSA refinement."""
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.cross_psa = CrossPSA(self.c, attn_ratio=attn_ratio, stripe_width=stripe_width)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Refine the final processed branch before the original C3k2 concatenation and fusion."""
        y = list(self.cv1(x).chunk(2, 1))
        y.extend(m(y[-1]) for m in self.m)
        y[-1] = self.cross_psa(y[-1])
        return self.cv2(torch.cat(y, 1))

    def forward_split(self, x: torch.Tensor) -> torch.Tensor:
        """Run the same CrossPSA path using split() instead of chunk()."""
